...
def classify_ad_link(link: str, competitors: list) -> str:
    """
    判斷是否為競品：
...
def append_ads_to_sheet(
    ...
    for idx, ad in enumerate(ads, start=1):
        # SerpApi 的廣告通常有 position，沒有就用 idx
        position = ad.get("position") or idx
        title = ad.get("title") or ""
        description = ad.get("description") or ad.get("snippet") or ""
        link = ad.get("link") or ad.get("tracking_link") or ""

        status = classify_ad_link(link, competitors)
    ...
    try:
        ads_ws.append_rows(rows, value_input_option="RAW")
    except AttributeError:
        for row in rows:
            ads_ws.append_row(row, value_input_option="RAW")


def main():
    print("[INFO] Starting Google Ads Monitor (SerpApi)...")
    ...
    ads_ws = get_or_create_worksheet(sh, "Ads")
    ...
    # 5. 逐一 Keyword 呼叫 SerpApi，寫入廣告結果
    for keyword in keywords:
        print(f"[INFO] Processing keyword (Ads): {keyword}")
        ...
        if ads:
            ...
            print(
                f"[INFO] Appended {len(ads)} ads "
                f"for keyword '{keyword}'."
            )
        else:
            print(f"[INFO] No ads found for keyword '{keyword}'.")

    print("[INFO] Google Ads Monitor script finished.")
